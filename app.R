...
# COMMENTS
{
  ...
  HOME_COMMENT8 = p(strong("Reference:"), "Gu, W., Koh, H., Jang, H., Lee, B., Kang, B. (2023). MiSurv: An integrative web cloud platform for user-friendly microbiome data analysis with survival responses. Microbiology Spectrum 11(3): e05059-22", style = "font-size:13pt")
  ...
  INPUT_PHYLOSEQ_COMMENT2 = p("You can download example microbiome data 'biom.Rdata' in the 'phyloseq' format.", br(), 
                              ...
                              br(),
                              p(strong("Data description:"), "This example data are the public gut microbiome data (Zhang et al. 2018) we used in our real data applications (Gu et al., 2023). The raw sequence data are deposited in QIITA (", tags$a(href = "https://qiita.ucsd.edu", "https://qiita.ucsd.edu"), ") with the ID number 10508 (", tags$a(href = "https://qiita.ucsd.edu/study/description/10508", "https://qiita.ucsd.edu/study/description/10508"), "). More detailed sample extraction and raw sequence data processing procedures are described in (Zhang et al. 2018; Gu et al., 2023). "), 
...
server = function(input, output, session) {
        ...
        output$ref_micloud <- renderUI({
          tagList(
            box(title = strong("Reference"), width = NULL, status = "primary", solidHeader = TRUE, 
                ...
                p("Gu W, Koh H, Jang H, Lee B, Kang B. MiSurv: An integrative web cloud platform for user-friendly microbiome data analysis with survival responses. Microbiology Spectrum 2023:11(3):e05059-22")
            )
          )
        })
    ...
    observeEvent(input$subgroup.a,{
      observeEvent(input$subgroup.sel.a,{
        ...
        if(input$subgroup.a == "Yes"){
            ...
            tagList(
              ...
              shinyjs::hidden(
                shiny::div(id = "covariates_variablesCoxA", style = "margin-left: 2%",
                           prettyCheckboxGroup("covariatesOptionsCoxA", "Please select covariate(s)", choices = c(options_bin, options_con), selected = NULL, status = "primary", width = '70%'))),
          ...
          options_bin <<- chooseData$prim_varsSurv[[1]][ !(chooseData$prim_varsSurv[[1]] %in% c(input$censor.select))]
          options_con <<- chooseData$prim_varsSurv[[2]][ !(chooseData$prim_varsSurv[[2]] %in% c(input$surv.Time.select))]
          ...
          output$covariatesCoxA <- renderUI({
            tagList(
              ...
              shinyjs::hidden(
                shiny::div(id = "covariates_variablesCoxA", style = "margin-left: 2%",
                           prettyCheckboxGroup("covariatesOptionsCoxA", "Please select covariate(s)", choices = c(options_bin, options_con), status = "primary",  width = '70%'))),
            ...
            tagList(
              ...
              shinyjs::hidden(
                shiny::div(id = "covariates_variablesCoxB", style = "margin-left: 2%",
                           prettyCheckboxGroup("covariatesOptionsCoxB","Please select covariate(s)", sort(c(options_bin, options_con)), status = "primary", "null", width = '70%'))),
              ...
              shinyjs::hidden(
                shiny::div(id = "covariates_variablesCoxT", style = "margin-left: 2%",
                           prettyCheckboxGroup("covariatesOptionsCoxT"," Please select covariate(s)", status = "primary",
                                               c(chooseData$nt.selected.bin, chooseData$nt.selected.con)[which(c(chooseData$nt.selected.bin, chooseData$nt.selected.con) != input$subgroup.sel.t)], width = '70%'))),
  ...
  observeEvent(input$run, {
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        infile$qc_biom = tryCatch(biom.cleanS(infile$biom,
                                              input$kingdom,
                                              ...
                                              follow = chooseData$follow.time), error = function(e) {  
        ...
        infile$qc_biomNA = tryCatch(biom.cleanSNA(infile$biom,
                                                  input$kingdom,
                                                  lib.size.cut.off = input$slider1,
                                                  mean.prop.cut.off = input$slider2/100,
                                                  rem.tax = rem.tax.complete, rem.tax.str = rem.tax.partial,
  ...
  observeEvent(input$surv.KMrun, {
    ...
    withProgress({
      ...
      Surv.bin.cat.ref.ori.out <- tryCatch(alpha.bin.cat.ref.ori.func(chooseData$sam.dat, input$primvar.Surv.select), error = function(e) {  
        message("No outcome is available!")
        showModal(modalDialog(div("No outcome is available!")))
        return(NULL)})
      
      
      sam_dat0 <- tryCatch(alpha.bin.cat.recode.func(chooseData$sam.dat, input$primvar.Surv.select, Surv.bin.cat.ref.ori.out,
                                                     rename.cats_ref, rename.cats_com), error = function(e) {  
        message("No outcome is available!")
        showModal(modalDialog(div("No outcome is available!")))
        return(NULL)})
      ...
      sam_dat0 <- tryCatch(sam_no_miss_cov(sam_dat0, input$covariatesOptions.surv), error = function(e) {  
        message("No outcome is available!")
        showModal(modalDialog(div("No outcome is available!")))
        return(NULL)})
      ...
      surv.dat0 <- tryCatch(surv.events.dat.func(sam_dat0, 
                                                 ...
                                                 chooseData$censor.select), error = function(e) {  
        message("No outcome is available!")
        showModal(modalDialog(div("No outcome is available!")))
        return(NULL)})
      ...
      if ( input$covariates.surv == "None") {
        ...
        surv.dat <-tryCatch(surv.events.dat.func2(sam_dat0, 
                                                  ...
                                                  input$covariatesOptions.surv), error = function(e) {  
          message("No outcome is available!")
          showModal(modalDialog(div("No outcome is available!")))
          return(NULL)}) 
        
        cox.model <- tryCatch(surv.Cox.fit.func2( surv.dat, input$covariatesOptions.surv), error = function(e) {  
          message("No outcome is available!")
          showModal(modalDialog(div("No outcome is available!")))
          return(NULL)})
        ...
        output$surv.plot1 = try(renderPlot({
  ...
  observeEvent(input$runbtn_bin,{
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        sam_dat <- try(alpha.bin.cat.recode.func(chooseData$sam.dat, input$primvar, alpha.bin.cat.ref.ori.out,
        ...
        if (input$chooseMethod == "Welch t-test" | input$chooseMethod == "Wilcoxon rank-sum test (default)") {
          if (input$chooseMethod == "Welch t-test") {
            ...
            t.test.out <- tryCatch(alpha.bin.t.test(alpha.results$bin.var, alpha.results$alpha_div), error = function(e) {  
              message("No outcome is available!")
              showModal(modalDialog(div("No outcome is available!")))
              return(NULL)})
            ...
            wilcox.test.out <- tryCatch(alpha.bin.wilcox.test(alpha.results$bin.var, alpha.results$alpha_div), error = function(e) {  
              message("No outcome is available!")
              showModal(modalDialog(div("No outcome is available!")))
              return(NULL)})
          ...
          output$box_plots = try(renderPlot({
          ...
          output$forest_plots = try(renderPlot({
  ...
  observeEvent(input$beta_runbtn_cross_bin,{
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        beta.bin.cat.ref.ori.out <- try(beta.bin.cat.ref.ori.func(chooseData$sam.dat, input$beta.primvar_cross), silent = TRUE) 
        beta.sam_dat.bin <- try(beta.bin.cat.recode.func(chooseData$sam.dat, input$beta.primvar_cross,
                                                     beta.bin.cat.ref.ori.out,
                                                     rename.catsbin_ref, rename.catsbin_com), silent = TRUE) 
        
        beta.sam_dat.bin <- try(sam_no_miss_cov(beta.sam_dat.bin, input$beta_covariatesOptions_bin), silent = TRUE) 
        ...
        if (input$beta_covariates_bin == "None") {
          ...
          if (input$beta_chooseMethod_bin == "MiRKAT") {
            ...
            beta.bin.out <- try(beta.bin.cat.ref.func(input$beta.primvar_cross,
                                                  rename.catsbin_ref, rename.catsbin_com,
                                                  beta.sam_dat.bin, Ds.Ks = ds.Ks$res), silent = TRUE) 
          ...
          if (is.null(input$beta_covariatesOptions_bin)) {
            ...
            beta_re_cov <- try(beta_no_miss_cov(beta.sam_dat.bin, ds.Ks$res,input$beta_covariatesOptions_bin), silent = TRUE) 
            
            beta.bin.cov.out <- try(beta.bin.cov.cat.ref.func(input$beta.primvar_cross,
                                                          rename.catsbin_ref, rename.catsbin_com,
                                                          input$beta_covariatesOptions_bin, beta.sam_dat.bin,
                                                          Ds.Ks = beta_re_cov), silent = TRUE) 
        ...
        if (input$beta_covariates_bin == "None") {
          beta.down.results$CS = tryCatch(mirkat.bin(beta.data.results$data.q.out), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          
          beta.plot.info <- tryCatch(mirkat.bin.plot1(beta.down.results$CS, beta.data.results$data.q.out), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          
          output$m2_beta_2d = try(renderPlot({
          ...
          beta.down.results$CS = tryCatch(mirkat.bin.cov(beta.data.results$data.q.out), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
  ...
  observeEvent(input$taxa_runbtn_bin,{
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        taxa.results$lib.size <- try(taxa.results$lib.size[names(taxa.results$lib.size) %in% rownames(chooseData$sam.dat)], silent = TRUE) 
        ...
        sam_dat <- try(taxa.bin.cat.recode.func(chooseData$sam.dat, input$primvar_taxa, taxa.bin.cat.ref.ori.out,
                                            rename.cats_ref, rename.cats_com), silent = TRUE) 
        
        if (input$covariates_taxa == "None") {
          if (input$chooseMethod_taxa == "Negative binomial regression") {
            taxa.bin.out <- try(taxa.bin.cat.ref.united.func(input$primvar_taxa, rename.cats_ref,
                                                         rename.cats_com, sam_dat, taxa = chooseData$taxa.out[["count"]]), silent = TRUE) 
            
          } else {
            taxa.bin.out <- try(taxa.bin.cat.ref.united.func(input$primvar_taxa, rename.cats_ref,
                                                         rename.cats_com, sam_dat, taxa = chooseData$taxa.out[[taxa.types$dataType]]), silent = TRUE) 
          }
          ...
          if (input$chooseMethod_taxa == "Negative binomial regression") {
            ...
            taxa.bin.cov.out <- try(taxa.bin.cov.cat.ref.united.func(input$primvar_taxa, rename.cats_ref, 
                                                                 rename.cats_com, input$covariatesOptions_taxa, 
                                                                 sam_dat, chooseData$taxa.out[["count"]]), silent = TRUE) 
            
            taxa.bin.cov.out <- try(taxa_no_miss_cov(sam_dat, taxa.bin.cov.out, input$covariatesOptions_taxa), silent = TRUE) 
          } else {
            taxa.bin.cov.out <- try(taxa.bin.cov.cat.ref.united.func(input$primvar_taxa, rename.cats_ref, 
                                                                 rename.cats_com, input$covariatesOptions_taxa, 
                                                                 sam_dat, chooseData$taxa.out[[taxa.types$dataType]]), silent = TRUE) 
        ...
        if (input$chooseMethod_taxa == "Welch t-test") {
          ...
          taxa.t.test.out <- tryCatch(taxa.bin.t.test.united(taxa.results$bin.var, taxa.results$taxa), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          
          taxa.t.test.q.out <- tryCatch(bin.q.united.func(taxa.t.test.out, method = "BH"), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          ...
          dend_result <- tryCatch(taxa.sig.dend(taxa.outputs$DAoutput, chooseData$NAadded$tax.tab, "twopi", include), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          ...
          taxa.wilcox.test.out <- tryCatch(taxa.bin.wilcox.test.united(taxa.results$bin.var, taxa.results$taxa), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          
          taxa.wilcox.test.q.out <- tryCatch(bin.q.united.func(taxa.wilcox.test.out, method = "BH"), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          taxa.wilcox.test.est.added <- tryCatch(taxa.wilcox.test.est.func(taxa.results$bin.var, taxa.results$taxa, rename.cats_ref, rename.cats_com, taxa.wilcox.test.q.out), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          ...
          output$boom_taxa= try(renderGrViz({
            taxa.sig.dend(taxa.outputs$DAoutput, chooseData$NAadded$tax.tab, "twopi", include)$flow.text
          }), silent = TRUE) 
          ...
          if (input$covariates_taxa == "None") {
            ...
            if (input$chooseMethod_taxa =="Linear regression") {
              ...
              taxa.lm.bin.out <- tryCatch(taxa.bin.lm.united.func(bin.var = taxa.results$bin.var, taxa = taxa.results$taxa), error = function(e) {  
                message("No outcome is available!")
                showModal(modalDialog(div("No outcome is available!")))
                return(NULL)})
              
              taxa.lm.bin.q.out <- tryCatch(bin.q.united.func(taxa.lm.bin.out, method = "BH"), error = function(e) {  
                message("No outcome is available!")
                showModal(modalDialog(div("No outcome is available!")))
                return(NULL)})
              ...
              nrow <- tryCatch(taxa.forest.plot.pages(taxa.lm.bin.q.out, species.include = include), error = function(e) {  
                message("No outcome is available!")
                showModal(modalDialog(div("No outcome is available!")))
                return(NULL)})
              ...
              taxa.bin.glm.nb.q.out <- tryCatch(all.taxa.bin.glm.nb(taxa.results$bin.var, taxa.results$taxa, taxa.results$lib.size), error = function(e) {  
                message("No outcome is available!")
                showModal(modalDialog(div("No outcome is available!")))
                return(NULL)})
              ...
              nrow <- tryCatch(taxa.forest.plot.pages(taxa.outputs$DAoutput, species.include = include), error = function(e) {  
                message("No outcome is available!")
                showModal(modalDialog(div("No outcome is available!")))
                return(NULL)})
              ...
              taxa.bin.beta.q.out <- tryCatch(all.taxa.bin.beta(taxa.results$bin.var, taxa.results$taxa), error = function(e) {  
                message("No outcome is available!")
                showModal(modalDialog(div("No outcome is available!")))
                return(NULL)})
            ...
            if (input$chooseMethod_taxa =="Linear regression") {
              ...
              taxa.lm.bin.cov.out <- tryCatch(taxa.bin.cov.lm.united.func(bin.var = taxa.results$bin.var, 
                                                                          cov.var = taxa.results$cov.var,
                                                                          taxa = taxa.results$taxa), error = function(e) {  
                message("No outcome is available!")
                showModal(modalDialog(div("No outcome is available!")))
                return(NULL)})
              
              taxa.lm.bin.cov.q.out <- tryCatch(bin.q.united.func(taxa.lm.bin.cov.out, method = "BH"), error = function(e) {  
                message("No outcome is available!")
                showModal(modalDialog(div("No outcome is available!")))
                return(NULL)})
              ...
              incProgress(5/10, message = "Negative binomial regression")
              taxa.bin.cov.glm.nb.q.out <- tryCatch(all.taxa.bin.cov.glm.nb(taxa.results$bin.var, taxa.results$cov.var, taxa.results$taxa, taxa.results$lib.size), error = function(e) {  
                message("No outcome is available!")
                showModal(modalDialog(div("No outcome is available!")))
                return(NULL)})
          ...
          forestplot.data <<- tryCatch(taxa.forest.plot.pages1(taxa.outputs$DAoutput, chooseData$taxa.names.out, report.type = "Est", species.include = include), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          ...
          lapply(1:nrow, function(j) {
            output[[paste0("M2forest", j)]] <- try(renderPlot({
              taxa.forest.plot.pages2(page.taxa.q.out = forestplot.data, page = j)  
            }), silent = TRUE) 
          })
          
          lapply(1:nrow, function(j) {
            output[[paste0("duplicates", j)]] <- try(renderPlot({
          ...
          first.taxa.sig <- tryCatch(taxa.sig.dend(taxa.outputs$DAoutput, chooseData$NAadded$tax.tab, "twopi", include), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          ...
          output$dend_taxo_rank = try(renderGrViz({
            taxa.sig.dend(taxa.outputs$DAoutput, chooseData$NAadded$tax.tab, "twopi", include)$flow.text
          }), silent = TRUE) 
          
          output$tax_r1 <- try(renderText({
            sig.tab1 <- kable(tab.one, 'html', booktabs =TRUE, escape = FALSE) %>%
              kable_styling(latex_options = c('hold_position'))
            sig.tab1
          }), silent = TRUE) 
          
          output$tax_r2 <- try(renderText({
            sig.tab2 <- kable(tab.two, 'html', booktabs =TRUE, escape = FALSE) %>%
              kable_styling(latex_options = c('hold_position'))
            sig.tab2
          }), silent = TRUE) 
          
          output$tax_r3 <- try(renderText({
            sig.tab3 <- kable(tab.three, 'html', booktabs =TRUE, escape = FALSE) %>%
              kable_styling(latex_options = c('hold_position'))
            sig.tab3
          }), silent = TRUE) 
          
          output$tax_r4 <- try(renderText({
            sig.tab4 <- kable(tab.four, 'html', booktabs =TRUE, escape = FALSE) %>%
              kable_styling(latex_options = c('hold_position'))
            sig.tab4
          }), silent = TRUE) 
          
          output$tax_r5 <- try(renderText({
            sig.tab5 <- kable(tab.five, 'html', booktabs =TRUE, escape = FALSE) %>%
              kable_styling(latex_options = c('hold_position'))
            sig.tab5
          }), silent = TRUE) 
          
          output$duplicates = try(renderPlot({
            duplicate.list(duplicate.taxa, taxon.inplot, chooseData$taxa.names.out$duplicates)
          }), silent = TRUE) 
  ...
  observeEvent(input$runbtn_CoxA, {
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        new_data <- try(data.union.func(chooseData$sam.dat, chooseData$alpha.div), silent = TRUE) 
        ...
        new_alpha.div <- try(alpha_no_miss_cov(new_sam.dat,  new_alpha.div, input$covariatesOptionsCoxA), silent = TRUE) 
        new_sam.dat <- try(sam_no_miss_cov(new_sam.dat, input$covariatesOptionsCoxA), silent = TRUE) 
        
        new_surv.dat <- try(alpha.surv.events.dat.func(new_sam.dat, chooseData$surv.Time.select, chooseData$censor.select), silent = TRUE) 
        
        if (input$covariatesCoxA =="None") {
          alpha.cox.out <- tryCatch(alpha.cox.test(new_surv.dat, new_alpha.div), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
        }
        else {
          cov.dat <- try(subset(new_sam.dat, select = input$covariatesOptionsCoxA), silent = TRUE) 
          alpha.cox.out <- tryCatch(alpha.cox.test(new_surv.dat, new_alpha.div, cov.dat), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
        }
        ...
        alphaS.down.results$alpha.cox.forest.plot <- try(alpha.cox.forest.plot(alphaS.down.results$alpha.cox.out, multi.test = multi.test), silent = TRUE) 
  ...
  observeEvent(input$runbtn_CoxB, {
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        beta.bin.cat.ref.ori.out <- try(beta.bin.cat.ref.ori.func(chooseData$sam.dat, betaS.primvar_cross), silent = TRUE) 
        
        rename.catsbin_ref = beta.bin.cat.ref.ori.out[1]
        rename.catsbin_com = beta.bin.cat.ref.ori.out[2]
        
        beta.sam_dat.bin <- try(beta.bin.cat.recode.func(chooseData$sam.dat, betaS.primvar_cross,
                                                     beta.bin.cat.ref.ori.out,
                                                     rename.catsbin_ref, rename.catsbin_com), silent = TRUE) 
        
        beta.sam_dat.bin <- try(sam_no_miss_cov(beta.sam_dat.bin, input$covariatesOptionsCoxB), silent = TRUE) 
        
        ds.Ks.res <- ds.Ks$res
        ds.Ks.res <- try(beta_no_miss_cov(beta.sam_dat.bin, ds.Ks.res, input$covariatesOptionsCoxB), silent = TRUE) 
        ...
        if (input$covariatesCoxB == "None") {
          betaS.bin.out <- tryCatch(betaS.bin.cat.ref.func(betaS.primvar_cross, betaS.primvar_time,
                                                           rename.catsbin_ref, rename.catsbin_com,
                                                           beta.sam_dat.bin, Ds.Ks = ds.Ks.res), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          ...
          if (is.null(input$covariatesOptionsCoxB)) {
            ...
            betaS.bin.cov.out <- tryCatch(betaS.bin.cov.cat.ref.func(betaS.primvar_cross, betaS.primvar_time,
                                                                     rename.catsbin_ref, rename.catsbin_com,
                                                                     input$covariatesOptionsCoxB, beta.sam_dat.bin,
                                                                     Ds.Ks = ds.Ks.res), error = function(e) {  
              message("No outcome is available!")
              showModal(modalDialog(div("No outcome is available!")))
              return(NULL)})
        ...
        if (isolate(input$covariatesCoxB) == "None") {
          ...
          betaS.down.results$CS = tryCatch(mirkatS.bin(betaS.data.results$data.q.out), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          
          
          betaS.plot.info <- tryCatch(mirkatS.bin.plot1(betaS.down.results$CS, betaS.data.results$data.q.out), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          
          
          output$m3_beta_2d = try(renderPlot({
          ...
          betaS.down.results$CS = tryCatch(mirkatS.bin.cov(betaS.data.results$data.q.out), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        if (input$covariatesCoxTaxa =="None") {
          taxa.cox.out <<- tryCatch(taxa.cox.test(survtime, status, taxa.out.surv), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
          ...
          taxa.cox.out <<- tryCatch(taxa.cox.test(survtime, status, taxa.out.surv, cov.dat = taxa.sam.dat[,input$covariatesOptionsCoxT]), error = function(e) {  
            message("No outcome is available!")
            showModal(modalDialog(div("No outcome is available!")))
            return(NULL)})
        }
        
        nrow <- tryCatch(surv.taxa.forest.plot.pages(taxa.cox.out, include, mult.test.cor = "TRUE"), error = function(e) {  
          message("No outcome is available!")
          showModal(modalDialog(div("No outcome is available!")))
          return(NULL)})
        
        forestplot.data <<- tryCatch(surv.taxa.forest.plot.pages1(taxa.cox.out, taxa.names.out.surv, report.type = "HR", species.include = include, mult.test.cor = TRUE), error = function(e) {  
          message("No outcome is available!")
          showModal(modalDialog(div("No outcome is available!")))
          return(NULL)})
        ...
        if (any(!is.na(unlist(taxa.names.out.surv$duplicates)))) {
          duplicate.taxa <- try(sapply(strsplit(unlist(taxa.names.out.surv$duplicates), " :"),  "[", 1), silent = TRUE) 
          taxon.inplot <- unlist(lapply(forestplot.data$all.text.tab, `[`, i =, j = 3))
          duplicate.texts <- sum(duplicate.taxa %in% taxon.inplot)
        } else {
          duplicate.texts <- 0
        ...
        lapply(1:nrow, function(j) {
          output[[paste0("forest_Surv", j)]] <- try(renderPlot({
            taxa.surv.forest.plot.pages2(page.taxa.q.out = forestplot.data, page = j)
          }), silent = TRUE) 
        })
        
        lapply(1:nrow, function(j) {
          output[[paste0("duplicates_surv", j)]] <- try(renderPlot({
            duplicate.list(duplicate.taxa, taxon.inplot, chooseData$taxa.names.out$duplicates)
          }), silent = TRUE) 
        })
        
        taxa.sig <- tryCatch(taxa.sig.dend(taxa.cox.out, chooseData$NAadded$tax.tab, "twopi", include), error = function(e) {  
          message("No outcome is available!")
          showModal(modalDialog(div("No outcome is available!")))
          return(NULL)})
  ...
  observeEvent(input$runbtn_model4, {
    ...
    taxa.sam.dat <- chooseData$sam.dat[match(rownames(chooseData$taxa.out$clr$phylum),rownames(chooseData$sam.dat)),]
    surv.dat <- try(surv.events.dat.func(taxa.sam.dat, 
                                     surv.con = chooseData$surv.Time.select, 
                                     surv.second.bin = chooseData$censor.select), silent = TRUE) 
