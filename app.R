...
# COMMENTS
{
  ...
  HOME_COMMENT = p(strong("MiCloud-S", style = "font-size:15pt"), "is an integrative web cloud platform for processing, analyzing and visualizing microbiome data with survival responses. 
                  MiCloud-S consists of five main modules: 1) Data Processing, 2) Module 1: Comparative survival analysis between treatment groups, 
                  3) Module 2: Comparative analysis in microbial composition between treatment groups, 
                  4) Module 3: Association testing between microbial composition and survival responses, 
                  5) Module 4: Prediction modeling using microbial taxa on survival responses. 
  ...
  HOME_COMMENT6 = p("Reference: Gu W, Koh H, Lee B, Jang HJ. MiCloud-S: An integrative web cloud platform for user-friendly microbiome data processing, analytic and graphical procedures with survival responses. (in progress)", style = "font-size:13pt")
      ...
      tabItems(
                ...
                sidebarLayout(
                  ...
                  sidebarPanel(width = 3,
                               ...
                               selectInput("censor.select", label = "",
                                           c("Choose one" = "", ""), selected = "", width = '70%'),
                               
                               
                               p(" ", style = "margin-bottom: -10px;"),
                               
                               
                               sliderInput("follow.time", h4(strong("Adjust Follow-up Period?")), min=0, max=20, value = c(0,15), step = 1),
                               
                               
                               prettyRadioButtons("subgroup",label = h4(strong("Subgroup Analysis?", style = "color:black")), status = "primary", icon = icon("check"),
                                                  animation = "jelly", c("No", "Yes"), selected="No", width = '70%'),
                               p(" ", style = "margin-bottom: -25px;"),
                               shinyjs::hidden(
                                 tagList(
                                   shiny::div(id = "subgroup.sel", style = "margin-left: 5%",
                                              selectInput("subgroup.sel", label = "", 
                                                          c("Choose one" = "", "null"), selected = "null", width = '70%'))
                                 )
                               ),
                               p(" ", style = "margin-bottom: -10px;"),
                               uiOutput("pick_subgroup"),
        ...
        tabItem(tabName = "taxaAnalysisSurv", br(),
                sidebarLayout(
                  ...
                  mainPanel(width = 9,
                            fluidRow(width = 12, 
                                     div(style='height:800px;overflow-y: scroll;', uiOutput("staxa_display_results")), br(),br(),
                                     
                                     uiOutput("staxa_display_dend"))
                            #column(9,  uiOutput("staxa_display_dend")),
                            #column(2, uiOutput("M3sig_taxon"))
                  ))),
        ##### Model 4: Prediction ####
...
server = function(input, output, session) {
  ...
  QC = observe(suspended = T,{
    ...
    observeEvent(input$censor.select, {
      observeEvent(input$subgroup,{
        if (input$subgroup == "Yes"){
          cat.for.subgroup = infile$ori.var[[1]][ ! infile$ori.var[[1]] %in% input$censor.select ]
          
          if(!is.null(chooseData$subgroup.sel)){
            if(chooseData$subgroup.sel %in% cat.for.subgroup) {
              updateSelectInput(session, "subgroup.sel", label = "", c("Choose one" = "", cat.for.subgroup), 
                                selected = chooseData$subgroup.sel)
            } else {
              updateSelectInput(session, "subgroup.sel", label = "", c("Choose one" = "", cat.for.subgroup), 
                                selected = cat.for.subgroup[1])
            } 
          } else {
            updateSelectInput(session, "subgroup.sel", label = "", c("Choose one" = "", cat.for.subgroup), 
                              selected = cat.for.subgroup[1])
          }
          shinyjs::show("subgroup.sel")
          shinyjs::show("pick_subgroup")
        }
        else {
    ...
    observeEvent(input$subgroup.sel, {
      if(input$subgroup.sel %in% infile$ori.var[[1]]){
        
        subgroup.val <- alpha.bin.cat.ref.ori.func(sample_data(infile$biom), input$subgroup.sel)
        
        output$pick_subgroup <- renderUI({
          tagList(
            p(" ", style = "margin-bottom: -5px;"),
            shiny::div(id = "pick_subgroup", style = "margin-left: 5%",
                       radioButtons("pick_subgroup", label =  "Please select a subgroup:", 
  ...
  observeEvent(infile$biom, {
    ...
    updateSelectInput(session, "surv.Time.select", choices = c("Choose one" = "", infile$ori.var[[2]]), selected = infile$ori.var[[2]][1])
    updateSelectInput(session, "censor.select", label = "", c("Choose one" = "", infile$ori.var[[1]]), selected = infile$ori.var[[1]][1])
  })
    ...
    observeEvent(chooseData$prim_varsSurv, {
      ...
      if (length(chooseData$prim_varsSurv[[1]]) > 0){
        ...
        output$primvarsSurv <- renderUI({
          tagList(
            selectInput("primvar.Surv.select", label = h4(strong("Treatment?", style = "color:black")),
                        c("Choose one" = "", chooseData$nt.selected.bin), selected = chooseData$nt.selected.bin[1], width = '70%'))
        })
      ...
      observeEvent(input$primvar.Surv.select, {
        ...
        if(!is.null(input$pick_KM_Cox)) {
          if(input$pick_KM_Cox == "Kaplan-Meier analysis") {
          ...
          output$chooseTest.surv <- renderUI({
            tagList(
              ...
              radioButtons("pick_KM_Cox", label = "", #label = h4(strong("Method?", style = "color:black")),
                           choices = c("Kaplan-Meier analysis", "Cox model"),
                           selected = "Kaplan-Meier analysis", width = '80%')
            )
          })
        }
  ...
  observeEvent(input$pick_KM_Cox, {
    if(input$pick_KM_Cox == "Kaplan-Meier analysis") {shinyjs::show("chooseTest.surv2")}
      ...
      tagList(
        ...
        shinyjs::hidden(
          shiny::div(id = "covariates_variablesCoxA", style = "margin-left: 2%",
                     prettyCheckboxGroup("covariatesOptionsCoxA"," Please select covariate(s)", status = "primary",
        ...
        prettyRadioButtons("chooseAdjustment3A", label = h4(strong("Multiple Testing Adjustment?", style = "color:black")), c("Yes", "No (Default)"), selected = "No (Default)",
      ...
      tagList(
        ...
        shinyjs::hidden(
          shiny::div(id = "covariates_variablesCoxB", style = "margin-left: 2%",
                     prettyCheckboxGroup("covariatesOptionsCoxB"," Please select covariate(s)", status = "primary",
                                         c(chooseData$nt.selected.bin, chooseData$nt.selected.con), width = '70%'))),
        ...
        radioButtons("surv3.beta.method", label = "",#h4(strong("Method?", style = "color:black")),
                     c("MiRKATS"), selected = "MiRKATS", width = '80%')
      )
    })
                ...
                if (!is.null(input$covariatesOptions)) {
                  ...
                  output$chooseTest <- renderUI({
                    tagList(
                      ...
                      radioButtons("chooseMethod", label = "",#h4(strong("Method?", style = "color:black")),
                                   c("Welch t-test", "Wilcoxon rank-sum test"), selected = "Welch t-test", width = '80%'))
                  })
                }
              })
  ...
  observeEvent(chooseData$taxa.out,{
    ...
    output$primvars_taxa <- renderUI({
      tagList(
        prettyRadioButtons("dataType_taxa", label = h4(strong("Data Format?", style = "color:black")), animation = "jelly",
                           c("CLR (Default)", "Count", "Proportion", "Arcsine-root"), selected = "CLR (Default)",width = '70%'),
        p(" ", style = "margin-bottom: -10px;"),
        selectInput("primvar_taxa", label = h4(strong("Primary Variable?", style = "color:black")),
                    choices = chooseData$nt.selected.bin, selected = chooseData$nt.selected.bin[1], width = '70%'))
    })
                ...
                if (!is.null(input$covariatesOptions_taxa)) {
                  ...
                  output$chooseTest_taxa <- renderUI({
                    tagList(
                      ...
                      radioButtons("chooseMethod_taxa", label = "",#h4(strong("Method?", style = "color:black")), 
                                   c("Welch t-test", "Wilcoxon rank-sum test",taxa.types$regression),
    ...
    output$model4_data_format <- renderUI({
      tagList(
        ...
        radioButtons("surv4.method.select", label = "",#h4(strong("Method?", style = "color:black")),
                     c("Regularized Cox model", "Random survival forest"), selected = "Regularized Cox model", width = '70%'),
    ...
    observeEvent(input$surv4.method.select, {
      if (input$surv4.method.select == "Random survival forest") {
        shinyjs::show("surv4.trees")
        shinyjs::hide("surv4.fold.select")
        output$surv4.trees <- renderUI({
          tagList(
            p(" ", style = "margin-top: -10px;"),
            selectInput("surv4.tree.select", label = h4(strong("Number of Trees?", style = "color:black")),
                        c("Choose one" = "", c(3000, 5000)), selected = "3000", width = '70%')
          )
        })
        ...
        output$surv4.num.display <- renderUI({
          tagList(
            p(" ", style = "margin-top: 5px;"),
            p(strong("Number of Predictors?"), style = "font-size:13.5pt; color:black"),
            p(" ", style = "margin-bottom: -5px;"),
            Num_PredComment,
            p(" ", style = "margin-bottom: -20px;"),
    ...
    observeEvent(input$surv4.dataType_taxa,{
      if (input$surv4.dataType_taxa == "Count") {
        taxa.types$dataType_model4 = "rare.count"
    ...
    observeEvent(input$surv.dataType_taxa,{
      if (input$surv.dataType_taxa == "Count") {
        taxa.types$dataType_Surv = "count"
  ...
  observeEvent(input$runbtn_CoxA, {
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        ref_string = REFERENCE_CHECK(method_name = isolate(input$surv3.alpha.method), FDR = isolate(input$chooseAdjustment3A))
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        if (input$covariatesCoxT =="None") {
          taxa.cox.out <- taxa.cox.test(survtime, status, chooseData$taxa.out[[taxa.types$dataType_Surv]])
        }
        else {
          taxa.cox.out <- taxa.cox.test(survtime, status, chooseData$taxa.out[[taxa.types$dataType_Surv]], cov.dat = taxa.sam.dat[,input$covariatesOptionsCoxT])
        }
        
        nrow <- surv.taxa.forest.plot.pages(taxa.cox.out, include, mult.test.cor = "TRUE")
        forestplot.data <- surv.taxa.forest.plot.pages1(taxa.cox.out, chooseData$taxa.names.out, report.type = "HR", species.include = include, mult.test.cor = TRUE)
        ...
        if (duplicate.texts > 0) {
          output$staxa_display_results = renderUI({
            tagList(
        ...
        output$staxa_display_dend = renderUI({
  ...
  observeEvent(input$runbtn_model4, {
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        for(i in 1:ranks) {
          taxa.dat[[i]] <- taxa.dat[[i]][rownames(surv.dat),]
        }
        
        if (input$surv4.method.select == "Random survival forest") {
          incProgress(6/10, message = "Random survival forest")
          rf.fit.out <- try(surv.random.forest(taxa.dat, chooseData$taxa.names.out, surv.dat, n.tree = trees, ranks.upto = ranks), silent = TRUE)
        ...
        else if (input$surv4.method.select == "Regularized Cox model") {
          for (j in 1:ranks) {
            incProgress(1/10, message = "Regularized Cox model")
            out[[j]] <- try(surv.en.reg.fit(taxa.dat, surv.dat, chooseData$taxa.names.out, j, n.folds = folds, loss.func = "C"), silent = TRUE)
          }
          ...
          for (k in 1:ranks) {
            if (sum(is.na(chooseData$taxa.names.out$duplicates[[k]])) == 0) {
              duplicate.taxa <- sapply(strsplit(unlist(chooseData$taxa.names.out$duplicates[[k]]), " :"),  "[", 1)
              taxon.inplot <- unlist(out[[k]]$sig.coef.out$Taxon)
              duplicates[[k]] <- duplicate.taxa %in% taxon.inplot
          ...
          output$surv4_display_results = renderUI({
            tagList(
              do.call(tabsetPanel, lapply(1:ranks, function(i) {
                tabPanel(title = rank[i], align = "center",
                         tabsetPanel(tabPanel(title = "Coefficient", align = "center",
                                              plotOutput(paste0("importance", i), height = 600, width = 500),
                                              plotOutput(paste0("duplicates_m4", i), height = 11.5*sum(duplicates[[i]])+10, width = 500)),
                                     tabPanel(title = "Supplements", align = "center",
                                              plotOutput(paste0("supplements", i), height = 600, width = 500)))
                )
              }))
            )
          })
          ...
          lapply(1:ranks, function(j) {
            output[[paste0("supplements", j)]] <- renderPlot({
              par(mfrow = c(2,1))
              par(mar = c(5,9,5,5))
              plot(out[[j]]$cv.kcv)
              plot(out[[j]]$lasso.kcv)
            })
          })
          ...
          output$m4tdownloadTabl <- downloadHandler(
            ...
            content = function(DA.file) {
              ...
              on.exit(setwd(temp))
              if(ranks == 5) {
                dataFiles = c("Phylum.csv", "Class.csv", "Order.csv" ,"Family.csv", "Genus.csv")
                write.csv(as.data.frame(out[[1]]$coefficient), file = "Phylum.csv")
                write.csv(as.data.frame(out[[2]]$coefficient), file = "Class.csv")
                write.csv(as.data.frame(out[[3]]$coefficient), file = "Order.csv")
                write.csv(as.data.frame(out[[4]]$coefficient), file = "Family.csv")
                write.csv(as.data.frame(out[[5]]$coefficient), file = "Genus.csv")
              }
              else if(ranks == 6) {
                dataFiles = c("Phylum.csv", "Class.csv", "Order.csv" ,"Family.csv", "Genus.csv", "Species.csv")
                write.csv(as.data.frame(out[[1]]$coefficient), file = "Phylum.csv")
                write.csv(as.data.frame(out[[2]]$coefficient), file = "Class.csv")
                write.csv(as.data.frame(out[[3]]$coefficient), file = "Order.csv")
                write.csv(as.data.frame(out[[4]]$coefficient), file = "Family.csv")
                write.csv(as.data.frame(out[[5]]$coefficient), file = "Genus.csv")
                write.csv(as.data.frame(out[[6]]$coefficient), file = "Species.csv")
              }
              zip(zipfile=DA.file, files=dataFiles)
            }
          )
  ...
  observeEvent(input$run, {
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        if (input$subgroup == "Yes") {
          chooseData$nt.selected.bin <- chooseData$prim_varsSurv[[1]][ !chooseData$prim_varsSurv[[1]] %in% c(input$censor.select, input$subgroup.sel)]
          chooseData$nt.selected.con <- chooseData$prim_varsSurv[[2]][ !chooseData$prim_varsSurv[[2]] %in% c(input$surv.Time.select)]
        } else if (input$subgroup == "No") {
