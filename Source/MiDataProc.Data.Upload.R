...
REFERENCE_CHECK <- function(data_transform = "", method_name = "", FDR = "", sig_test = ""){
  ...
  # else if(data_transform == "Arcsin Root"){
  ...
  else if(method_name == "Kaplan-Meier analysis"){
    reference_lists = c(reference_lists, 
  ...
  else if(method_name == "Random survival forest") {
    reference_lists = c(reference_lists, "Mogensen UB, Ishwaran H, Gerds TA. Evaluating random forests for survival analysis using prediction error curves. J Stat Softw. 2012;50(11):1-23")
  }
  ...
  else if(method_name == "MiRKATS") {
    reference_lists = c(reference_lists, "Koh H, Livanos AE, Blaser MJ, Li H. A highly adaptive microbiome-based association test for survival traits. BMC Genomics. 2018;19(210)",
